...
public record FormacaoDTO(Long id, String titulo, String descricao, String status, CandidatoDTO candidatoDTO) implements Serializable {
    @Serial
    private static final long serialVersionUID = 1L;

    public static Formacao toEntity(FormacaoDTO formacaoDTO) {
        if (formacaoDTO == null) return null;
        return new Formacao(formacaoDTO.titulo(), formacaoDTO.descricao(), formacaoDTO.status(), CandidatoDTO.toEntity(formacaoDTO.candidatoDTO));
    }
    public static FormacaoDTO toDTO(Formacao formacao) {
        if (formacao == null) return null;
        return new FormacaoDTO(formacao.getId(), formacao.getInstituicao(), formacao.getCurso(), formacao.getStatus(), CandidatoDTO.toDTO(formacao.getCandidato()));
    }
